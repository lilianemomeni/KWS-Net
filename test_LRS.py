...
def evaluation(config, logger=None):
    ...
    for j, batchword in enumerate(Words):
      for i, lstVwidx in enumerate(test_loader):
        ...
        values = preds['max_logit'].view(1, len(target)).detach().cpu().numpy()[0]
        ground_truths = target.detach().cpu().numpy()
        original_labels = np.concatenate((original_labels, ground_truths), axis=0)
    ...
    for i in range(0,644):
      ...
      map_all_words.append(mean_average_precision_score(labels_word,scores_word,original_labels_word))
      Iscores = np.argsort(-scores_word)
      labels_word = labels_word[Iscores]
      recall_k_all_words_1.append(recall_at_k(labels_word,1, original_labels_word))
      recall_k_all_words_5.append(recall_at_k(labels_word,5, original_labels_word))
      recall_k_all_words_10.append(recall_at_k(labels_word,10, original_labels_word))
